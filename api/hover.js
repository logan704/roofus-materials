...
var HOVER_API_HOST = "api.hover.to";
var HOVER_AUTH_HOST = "hover.to";
...
function sbGet(table, query) {
  return httpReq({
    hostname: SB_HOST, path: "/rest/v1/" + table + "?" + query, method: "GET",
...
function sbUpsert(table, data) {
  ...
  return httpReq({
    hostname: SB_HOST, path: "/rest/v1/" + table, method: "POST",
...
function saveTokens(tokens) {
  return sbUpsert("app_settings", { key: "hover_tokens", value: JSON.stringify(tokens), updated_at: new Date().toISOString() });
}

function hoverTokenExchange(bodyObj) {
  var body = JSON.stringify(bodyObj);
  return httpReq({
    hostname: HOVER_AUTH_HOST, path: "/oauth/token", method: "POST",
...
function hoverGet(path, token) {
  return httpReq({
    hostname: HOVER_API_HOST, path: "/api/v2" + path, method: "GET",
...
module.exports = function(req, res) {
  ...
  if (action === "ping") {
    return res.status(200).json({ ok: true, v: 2 });
  }
  ...
  if (action === "jobs") {
    getValidToken().then(function(t) {
      ...
      var p = "/jobs?per_page=20&state=complete" + (s ? "&search=" + encodeURIComponent(s) : "");
      return hoverGet(p, t).then(function(d) { res.json(d); });
    }).catch(function(e) { res.status(500).json({ error: e.message }); });
    return;
  }
