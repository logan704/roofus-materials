...
export default async function handler(req, res) {
  ...
  try {
    // ─── FETCH JOBS ───
    if (action === "jobs") {
      const r = await fetch(`${BASE}/jobs?select=display_name,number,address_line1,address_line2,city,state_text,zip,status_name,record_type_name&limit=200&sort_field=date_updated&sort_direction=desc`, { headers });
    ...
    // ─── FETCH CONTACTS ───
    if (action === "contacts") {
      const r = await fetch(`${BASE}/contacts?select=display_name,first_name,last_name,address_line1,city,state_text,zip,status_name&limit=200&sort_field=date_updated&sort_direction=desc`, { headers });
      if (!r.ok) return res.status(r.status).json({ error: "JobNimbus API error", status: r.status });
      const data = await r.json();
